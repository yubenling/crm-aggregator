...
public class TmcRefundRefuseService {
	...
	public void doHandle(JSONObject content) throws Exception{
		...
		Long uid = user.getId();
		Map<Object,Object> tradeCreateSetupMaps = this.cacheService.hGetAll(OrderSettingInfo.TRADE_SETUP+uid+"_"+OrderSettingInfo.REFUND_REFUSE,true); 
		List<TradeSetup> list = TmcDistributeService.sortTradeSetup(tradeCreateSetupMaps.values());
		if(ValidateUtil.isEmpty(list)){
			this.logger.info("拒绝退款关怀未开启, " + content);
			return ;
		}
		...
		this.logger.info("拒绝退款开始处理  " + tmcMessages.getTid()+" ，用户设置了"+list.size()+"个任务");
		for (TradeSetup tradeSetup : list) {
			try {
				...
				this.logger.info("拒绝退款流程处理完，tid: " + tmcMessages.getTid()+ "处理结果:"+tmcMessages.getFlag()+" ,id:"+tmcMessages.getTradeSetup().getId());
