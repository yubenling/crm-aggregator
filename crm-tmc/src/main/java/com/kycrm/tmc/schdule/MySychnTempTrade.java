...
import com.kycrm.tmc.thread.DetailRFMQueueService;
import com.kycrm.tmc.thread.StandardRFMQueueService;
...
public class MySychnTempTrade {
	...
	@Autowired
	private StandardRFMQueueService standardRFMQueueService;
	
	@Autowired
	private DetailRFMQueueService detailRFMQueueService;
...
//						IMsgTempTradeQueueService.queue.put(msgSendRecord);
	...
	public void standardRFMJob(){
		...
		if(uidList != null && !uidList.isEmpty()){
			for (Long uid : uidList) {
				...
				try {
					StandardRFMQueueService.getQueue().put(uid);
			...
			standardRFMQueueService.handleLogAccessData();
		}
		TaskNode taskNode = taskNodeService.queryTaskNodeByType("standardRFM");
		if(taskNode != null){
			taskNode.setTaskEndTime(startNodeDate);
			taskNodeService.updateTaskNode(taskNode);
		}else {
			TaskNode newTaskNode = new TaskNode();
			newTaskNode.setTaskEndTime(startNodeDate);
			newTaskNode.setType("standardRFM");
			taskNodeService.saveTaskNode(newTaskNode);
		}
	...
	public void detailRFMJob() {
		logger.info("RFMCHART1111111111111111111111111111");
		List<Long> uidList = userInfoService.listTokenNotNull();
		logger.info("RFMCHART222222222222222222222222222");
		for (Long uid : uidList) {
			logger.info("RFMCHART23333333333333333333333333333333");
			try {
				DetailRFMQueueService.getQueue().put(uid);
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		}
		detailRFMQueueService.handleLogAccessData();
			
		TaskNode taskNode = taskNodeService.queryTaskNodeByType("detailChartRFM");
		if(taskNode != null){
			taskNode.setTaskEndTime(new Date());
			taskNodeService.updateTaskNode(taskNode);
		}else {
			TaskNode newTaskNode = new TaskNode();
			newTaskNode.setTaskEndTime(new Date());
			newTaskNode.setType("detailChartRFM");
			taskNodeService.saveTaskNode(newTaskNode);
		}
