...
import com.kycrm.tmc.util.CacheManagerUtil;
import com.kycrm.tmc.util.FastJSONUtil;
...
//		}
	...
	/**
	 * TODO 判断内存缓冲中有没有key,有则返回，没有则从redis中取并设置到redis中
	...
	public boolean getByName(String cacheName) {
		...
		if(CacheManagerUtil.hasKey(cacheName))return true;
		Boolean hasKey = redisTemplate.hasKey(projectName + cacheName);
		if(hasKey)CacheManagerUtil.setData(cacheName,redisTemplate.opsForHash().entries(cacheName),60);
		return hasKey;
	}
	...
	public <T> Map<String, T> getAllKeysAndValues(String cacheName, Class<T> className) {
		...
		Set<Object> keys = redisTemplate.opsForHash().keys(projectName + cacheName);
		Map<String, T> datas = new HashMap<String, T>(15);
		for (Object obj : keys) {
	...
	protected String jsonSuccess(Object mapObject) {
		Map<String, Object> allMap = new HashMap<String, Object>(15);
	...
	/**
	 * 删除内存中的key和redis中的key
	 * @param key  
	 * @param isDelete 是否删除内存中的缓存
	 */
	public void delete(String key,boolean isDelete) {
		redisTemplate.delete(key);
		if(isDelete)CacheManagerUtil.clear(key);
	}
