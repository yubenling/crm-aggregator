...
public class SmsRecordDTO extends BaseMongoDTO implements Serializable, Comparable<SmsRecordDTO> {
	...
	// + "29-买家申请退款 30-退款成功 31-等待退货 32-拒绝退款 33-会员短信群发 34-指定号码群发 35-订单短信群发
	// 36-会员互动")
	private String type;

	// @MetaData(value="发送状态 1：发送失败，2：发送成功，3：手机号码不正确，4：号码重复， 5 ：黑名单， 6 ：重复被屏蔽
	// /重复发送")
	...
	@Override
	public int compareTo(SmsRecordDTO smsRecordDTO) {
		if (sendTime.compareTo(smsRecordDTO.getSendTime()) == -1) {
			return -1;
		} else if (sendTime.compareTo(smsRecordDTO.getSendTime()) == 0) {
			return 0;
		} else {
			return 1;
		}
